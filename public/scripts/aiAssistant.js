...
(function () {
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape" && !root.classList.contains("ai-assistant--collapsed")) {
            closePanel();
        }
    });
    ...
    async function clearHistory() {
        if (!confirm("Clear the AI chat for the whole team? This can't be undone.")) {
            return;
        }
        try {
            const res = await fetch(`/api/projects/${projectId}/ai-chat`, {
